...
pub fn main() {
    ...
    let days: Vec<String> = read_dir(days_path).unwrap()
        .map(|e| e.unwrap().path())
    ...
    let returns: String = days.iter().map(|d| format!("crate::day::{}::get_day(),\n", d)).collect();

    f.write_all(format!(get_days_template!(), returns).as_bytes()).unwrap();
}
