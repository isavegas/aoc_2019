use aoc_core::{math, AoCDay, ErrorWrapper};
...
impl Body {
    ...
    fn energy(&self) -> usize {
        let p = self.position.x.abs() as usize
            + self.position.y.abs() as usize
            + self.position.z.abs() as usize;
        let k = self.velocity.x.abs() as usize
...
impl AoCDay for Day12 {
    ...
    fn part1(&self, input: &str) -> Result<String, ErrorWrapper> {
        ...
        Ok(format!(
            "{:?}",
            bodies.iter().map(|b| b.energy()).sum::<usize>()
        ))
    }
