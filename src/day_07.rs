...
impl AoCDay for Day07 {
    ...
    fn part1(&self) -> String {
        Heap::new(&mut [0, 1, 2, 3, 4]).map(|params| {
            let mut signal = 0;
            for p in &params {
                let mut machine = IntCodeMachine::new(INTCODE.clone(), vec![*p, signal]);
                assert!(machine.execute().expect("Machine crashed!") == ExecutionStatus::Halted, "Machine blocking!");
                signal = machine.output_buffer.pop().expect("No output from machine!");
            }
            signal
    ...
    fn part2(&self) -> String {
        ...
        Heap::new(&mut [5, 6, 7, 8, 9]).map(|params| {
            ...
            while !machines[4].halt {
                ...
                signal = machines[i].output_buffer.pop().expect("No output from machine!");
